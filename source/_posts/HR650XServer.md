...
又过了两天, 我把一张 1660 拿了过来, 上 tb 买了一根神秘供电线, 插上去了. 在检查的时候, 我发现这张卡的 PCIe 速率是 2.5GT/s (PCIe 1.0) 而不是 PCIe 3.0 . 我问了 GPT, GPT 给我了一大堆的建议, 然后我折腾了一个小时多, 没有成功.

我问叶先生, 第二天早上, 我看到叶先生说, 可能卡进入了节能模式, 跑一下负载试试. 结果还真是. 我把 hashcat 打开之后, 显卡的 PCIe 速率就变成 3.0 x16; 关掉之后又回到 1.0 x16. 太神奇了 ()

所以现在这也是一个 GPU 服务器了! (x)

(事实证明, 即便是 GPT 5.2 Thinking 也常常把这类问题归结于 BIOS 设置或者物理连接问题, 而不是 GPU 本身的节能模式)

## 串口

现在已经是 26 年 5 月了, 机器没有再死过. 这两天我又想起来串口的事情, 因为之前的 57600 的串口实在是慢, UEFI BIOS 会输出一大坨调试信息, 而串口是阻塞的 -- 串口的速度严重拖慢了我的开机速度. 于是我还是把 BIOS 里面改成了 115200.

我重新评估了一下之前的结论. 在把 BIOS 改成 115200 之后, POST 的输出变成了乱符 -- 这说明 POST log 的波特率应该与 BIOS 一致, 不是固定的 57600. 但是, 用 ipmitool 更改 SOL 波特率确实没有生效, 还是乱符. 我 warm reset + cold reset 了 BMC, 没用. Linux 的串口依然是 57600, 我进系统之后, 串口反倒是好了 -- 这说明 ipmi 的更改确实是没有生效, 当前的 SOL 依然是 57600, 与 ipmitool 的波特率不一致.

我发现 UEFI BIOS 里面有 SOL 设置, 这里的读数是 57600, 与 ipmitool 也不一致. 这显然说明了一些问题: 我设置的 ipmitool 和 BIOS 读取的不是一个东西. 我扫描了一遍 sol channels, 发现我可以从 channel 1, 4 和 14 获取配置, *默认修改的配置是 14*, 但是看上去生效的是 **1**. 运行:
