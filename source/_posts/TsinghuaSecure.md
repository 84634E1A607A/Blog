---
updated: 2025-06-07 23:14:50
title: Tsinghua-Secure - 802.1X 并非牢不可破
date: 2025-02-06 16:04:58
description: 本文探讨了清华大学校园无线网 Tsinghua-Secure 的安全性，指出其采用的 802.1X PEAP 认证方式存在漏洞，特别是由于证书验证被绕过，导致用户密码哈希可能被窃取。通过分析 MSCHAPv2 协议和伪基站攻击技术，文章展示了如何获取用户密码哈希，并通过字典爆破获取明文密码。实验结果显示，约10%的用户密码与主密码相同，可被用于未启用双因素认证的系统。结论是，Tsinghua-Secure 并非牢不可破，建议用户校验证书并信任系统证书以提高安全性。
tags:
  - 网络
  - 网络安全
---
...
我将此问题报告给了信息化的老师. 他们很重视这个问题, 正好去年准入认证系统更新了, 需要更新 SOP, 我就和他们研究了如何正确信任证书, 他们也发布了更正过这个问题的 SOP. 但是, 由于连上了网的同学和老师肯定不会关注更新的 SOP, 应该还是能打, 攻击面不小.

## 别人的实地攻击 (啊?)

今天惊闻某些不便透露姓名的同学看了这篇博客之后复现了这一攻击. 他们不知道弄了多久这玩意, 最后给我扔了一个截图:

总 **获取到的不同用户密码哈希数量约 3500 个** (真不少!)

他们通过多种手段, 利用字典 + 模式生成等手段生成密码, 进行爆破, 得到了 **约 1000 用户的明文密码**, 占比 30%.

他们对这些密码逐一进行了主 ID 登录测试, 得到 **约 350 个用户的密码与主密码相同, 可以登录未使用 2FA 的系统**, 占比 10%......
